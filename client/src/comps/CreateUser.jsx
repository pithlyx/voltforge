...
function CreateUser() {
	...
	const emailClass = email === verifyEmail ? 'border-green-300 bg-green-100' : 'border-red-300 bg-red-100';
	...
	}, [username, password, verifyPassword, email, verifyEmail, passStrength, userValid]);
		...
		if (Object.keys(errors).length === 0 && userValid) {
			...
			try {
				...
				if (error.message.includes('User')) {
					setValidationErrors((prevErrors) => ({ ...prevErrors, username: error.message }));
				}
				if (error.message.includes('Email')) {
					setValidationErrors((prevErrors) => ({ ...prevErrors, email: error.message }));
				}
			}
		}
	};
	...
	return (
		...
		>
			...
			<form className="p-6 rounded shadow-md" style={{ backgroundColor: '#94A3B8' }} onSubmit={handleCreate}>
				<div className="mb-4">
					<input
				...
				{validationErrors.password && <p className="text-red-500 text-xs mt-1 mb-4">{validationErrors.password}</p>}
				...
				{validationErrors.email && <p className="text-red-500 text-xs mt-1 mb-4">{validationErrors.email}</p>}
				...
				<div className="mb-4">
					...
					/>
					{validationErrors.cityName && <p className="text-red-500 text-xs mt-1">{validationErrors.cityName}</p>}
				</div>

				<input className="bg-amber-600 text-white p-2 rounded w-full" type="submit" value="Create User" />
			</form>
